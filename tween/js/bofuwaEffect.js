...
使用简介
	animate(ele,obj,nDuration,[effectType],[fnCallback])
	参数说明：
...
var bofuwaEffect = {
	...
	Bounce: {//指数衰减的反弹缓动。
		easeIn: function(t,b,c,d){
			return c - bofuwaEffect.Bounce.easeOut(d-t, 0, c, d) + b;
		},
		easeOut: function(t,b,c,d){
			if ((t/=d) < (1/2.75)) {
		...
		easeInOut: function(t,b,c,d){
			if (t < d/2) return bofuwaEffect.Bounce.easeIn(t*2, 0, c, d) * .5 + b;
			else return bofuwaEffect.Bounce.easeOut(t*2-d, 0, c, d) * .5 + c*.5 + b;
		}
	}
}
////////////////////以上都是算法
...
function animate(ele,obj,nDuration,effectType,fnCallback){
	...
	//effectType 1,2,3,4
	if(!effectType||effectType=="Linear"){
		var fn=bofuwaEffect.Linear;//直线，匀速的 1		
		...
		switch(effectType){
			case 1:
				...
				break;
			case 2:
				var fn=bofuwaEffect.Quart.easeInOut;//直线，匀速的 1
				break;
			case 3:
				var fn=bofuwaEffect.Elastic.easeOut;//直线，匀速的 1
				break;
			case 4:
				var fn=bofuwaEffect.Bounce.easeOut;//直线，匀速的 1
				break;
			default:
		...
		var fn=bofuwaEffect.Linear;	
		}else{			
			var fn=bofuwaEffect[effectType[0]][effectType[1]];		
		}		
